...
@property (nonatomic, strong) UIImageView *explosionImageView;        //爆炸动画view
...
- (void)explosion {
    ...
    self.explosionImageView.center = _touchPoint;
    [self.explosionImageView startAnimating];
    [self addSubview:self.explosionImageView];
    [self performSelector:@selector(endAnimation) withObject:self afterDelay:0.5];
}
...
- (UIImageView *)explosionImageView {
    if (!_explosionImageView) {
        NSMutableArray *array = [[NSMutableArray alloc] init];
        for (int i = 1; i < 6; i++) {
            UIImage *image = [[UIImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"id_%d", i] ofType:@"png"]];
            [array addObject:image];
        }
        _explosionImageView = [[UIImageView alloc] init];
        _explosionImageView.frame = CGRectMake(0, 0, 34, 34);
        _explosionImageView.animationImages = array;
        [_explosionImageView setAnimationDuration:0.5];
        [_explosionImageView setAnimationRepeatCount:1];
    }
    return _explosionImageView;
}
@end
