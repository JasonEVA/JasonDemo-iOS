...
// ************** 自备 **************//
static NSString * const password_iv  = @"zeusmint2013code";// 客户端密钥
static NSString * const password_ov  = @"zeusxiacheng2013";// 服务端密钥
static NSString * const password_usr = @"zeus-2013perfect";// 用户名、密码等参数的密钥
static NSString * const password_qr  = @"Mintcode.Launchr";
...
static Byte qrBuff[]   = {0xC, 1, 0xB, 3, 0x5B, 0xD, 5, 4, 0xF, 7, 9, 0x17, 1, 0xA, 6, 8};
...
{                  //Derive a key from a text password/passphrase
    ...
    NSData *salt = [NSData dataWithBytes:userBuff length:kCCKeySizeAES128];
...
/*加密方法*/
+ (NSString *)AES256EncryptWithPlainText:(NSString *)plain withKey:(Kinds)kind
{    
    ...
    switch (kind)
    {
        // 加密客服端用户名密码或者其他参数
        case KindUser:
            cryptStatus = CCCrypt(kCCEncrypt, kCCAlgorithmAES128,kCCOptionPKCS7Padding,
                                  [password_usr UTF8String], kCCKeySizeAES128,
                                  userBuff /* initialization vector (optional) */,
        ...
        case KindClient:
            cryptStatus = CCCrypt(kCCEncrypt, kCCAlgorithmAES128,kCCOptionPKCS7Padding,
                                  [password_iv UTF8String], kCCKeySizeAES128,
...
/*解密方法*/
+ (NSString *)AES256DecryptWithCiphertext:(NSString *)ciphertexts withKey:(Kinds)kind
{
    ...
    switch (kind)
    {
            // 解密客服端用户名密码或者其他参数
        case KindUser:
            cryptStatus = CCCrypt(kCCDecrypt, kCCAlgorithmAES128,kCCOptionPKCS7Padding,
                                  [password_usr UTF8String], kCCKeySizeAES128,
                                  userBuff /* initialization vector (optional) */,
        ...
        case KindServer:
            cryptStatus = CCCrypt(kCCDecrypt, kCCAlgorithmAES128,kCCOptionPKCS7Padding,
                                  [password_ov UTF8String], kCCKeySizeAES128,
                                  ovBuff /* initialization vector (optional) */,
                                  [cipherData bytes], dataLength, /* input */
                                  buffer, bufferSize, /* output */
                                  &numBytesDecrypted);
            break;
        case KindQRCode:
            cryptStatus = CCCrypt(kCCDecrypt, kCCAlgorithmAES128, kCCOptionPKCS7Padding,
                                  [password_qr UTF8String], kCCKeySizeAES128,
                                  qrBuff,
...
{
    if (string == nil)
        [NSException raise:NSInvalidArgumentException format:@"string can not be nil"];
    if ([string length] == 0)
        return [NSData data];
